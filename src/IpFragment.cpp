...
namespace FeatureExtractor {
	...
	bool IpFragment::get_ip_flag_df() const
	{
		return ip_flag_df;
	}

	void IpFragment::set_ip_flag_df(bool ip_flag_df)
	{
		this->ip_flag_df = ip_flag_df;
	}
	...
	void IpFragment::print() const
	{
		...
		if (get_eth_type() == IPV4) {
			cout<< "  ip.flag_df=" << get_ip_flag_df() 
				<< ", ip.flag_mf=" << get_ip_flag_mf()
