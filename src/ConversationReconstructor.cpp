...
namespace FeatureExtractor {
	...
	ConversationReconstructor::ConversationReconstructor()
		: timeouts()
		, timeout_interval(timeouts.get_conversation_check_interval_ms()) // num
	{
	}

	ConversationReconstructor::ConversationReconstructor(Config &timeouts)
		: timeouts(timeouts)											  // Config
		, timeout_interval(timeouts.get_conversation_check_interval_ms()) // 1000ms
	{
	}
	...
	void ConversationReconstructor::add_packet(const Packet *packet)
	{
		...
		if (!conversation) {
			...
			{
			case TCP:
				conversation = new TcpConnection(packet); // leak
				break;

			case UDP:
				conversation = new UdpConversation(packet); // leak
				break;

			case ICMP:
				conversation = new IcmpConversation(packet); // leak
				break;
