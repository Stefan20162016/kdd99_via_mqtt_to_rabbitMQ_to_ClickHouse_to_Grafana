...
static volatile bool termination_requested = false;
...
void extract(Sniffer *sniffer, const Config *config, bool is_running_live, TinyMqtt *tinymqtt_handle);
...
int main(int argc, char **argv)
{
	...
	TinyMqtt *tinymqtt_handle = new TinyMqtt("localhost", "1883", "kdd99features"); // host, port, topic
	//tinymqtt_handle->publish("tututut");
	//tinymqtt_handle->publish("tut2"); tinymqtt_handle->publish("tututut3"); tinymqtt_handle->publish("tututut4"); tinymqtt_handle->publish("lalalala");
	//return 0;
	...
	try {
		...
		if (config.get_files_count() == 0) {
			...
			extract(sniffer, &config, true, tinymqtt_handle);
		}
		else {
			// Input from files
			...
			for (int i = 0; i < count; i++) {
				...
				extract(sniffer, &config, false, tinymqtt_handle);
			}
		}
	}
...
void extract(Sniffer *sniffer, const Config *config, bool is_running_live, TinyMqtt * tinymqtt_handle)
{
	...
	while (!termination_requested && (has_more_traffic || is_running_live)) {

		// Get frame from sniffer
		IpFragment *frag = sniffer->next_frame();

		// debug output
		//std::cout << "DONTFRAGMENT:" << frag->get_ip_flag_df() << std::endl;
		//frag->print();
		...
		while ((conv = conv_reconstructor.get_next_conversation()) != nullptr) {
			...
			cf->print(config->should_print_extra_features()); // mainprint
			//std::cout << "mainCFeatures: " << cf -> get_count() << std::endl;

			std:: string s = cf-> prepare_mqttmessage();
			//tinymqtt_handle->publish(s.substr(0,10));
			//tinymqtt_handle->publish(s);
			TinyMqtt m(s);
			std::thread t{m};
			t.detach();
			std::cout << "preparedstring: " << s << std::endl;
			
			delete cf;
		}
		delete frag; // NEW DELETE
	}
